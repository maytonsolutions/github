...
CREATE EVENT hl7_export_records_chdallas_1000
    ON SCHEDULE
      EVERY 1 day
      STARTS '2018-03-09 15:00:00'
    COMMENT 'pick up every new records that are more than 10 seconds old'
    DO

BEGIN

        UPDATE LOW_PRIORITY hl7app.adt_msg_queue_chdallas
		    SET processing_status= 'p'
		    WHERE processing_status = 'r'
        AND customer_id = 'CHDALLAS'
        AND (msg_type = 'A03' || msg_type = 'A02')
        ...
        '' as 'Procedure2CPT',
        '' as 'Procedure3CPT',
        service_indicator01 as 'ServiceIndicator01'
	    FROM hl7app.adt_msg_queue_chdallas
		WHERE processing_status = 'p'
        AND customer_id = 'CHDALLAS' "
        ," into outfile 'C:/ProgramData/MySQL/MySQL Server 5.7/Uploads/CHDALLAS_HL7_"
        ...
        UPDATE hl7app.adt_msg_queue_chdallas
