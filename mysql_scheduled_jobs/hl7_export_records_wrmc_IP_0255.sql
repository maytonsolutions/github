...
CREATE EVENT hl7_export_records_wrmc_IP_0255
    ON SCHEDULE
      EVERY 1 day
      STARTS '2018-05-17 07:55:00'
    COMMENT 'pick up every new records that are more than 10 seconds old'
    DO
    
BEGIN

        UPDATE LOW_PRIORITY hl7app.adt_msg_queue_wrmc
		SET processing_status= 'p'
		WHERE processing_status = 'r'
        AND (customer_id = 'WRMC')
        AND visit_type = 'I'
        AND msg_type = 'A03'
        ...
        ," into outfile 'C:/ProgramData/MySQL/MySQL Server 5.7/Uploads/WRMC_HL7_IP"
        ...
        UPDATE hl7app.adt_msg_queue_wrmc
